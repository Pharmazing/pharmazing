...
export const HeroPromoCard = ({ title, description }: HeroPromoCardProps) => {
  ...
  return (
    <ImageBackground imageStyle={{borderRadius: theme.size.layout.lg -2}} resizeMode='cover' src={"https://picsum.photos/800/800"} style={[styles.itemContainer(isLgScreen)]}>
      <BlurView intensity={70} tint='light' style={styles.blurContainer} >
        <Typography size='lg' weight='500' style={{color: 'white'}}>{title}</Typography>
        <Typography style={{color: theme.colors.white}}>{description}</Typography>
      </BlurView>
    </ImageBackground>
  );
};
